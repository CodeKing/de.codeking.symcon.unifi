...
class Kint
{
    ...
    public static function trace($trace = null)
    {
        ...
        return self::dumpArray(
            [$trimmed_trace],
            [Kint_Object::blank('Kint::trace()', 'debug_backtrace()')]
        );
    }
    ...
    public static function dump($data = null)
    {
        ...
        if (!self::$dump_array && (!isset($params[0]['name']) || $params[0]['name'] == '1') && $num_args === 1 && $data === 1) {
            ...
            static $blacklist = ['null', 'true', 'false', 'array(...)', 'array()', '"..."', 'b"..."', '[...]', '[]', '(...)', '()'];
    ...
    private static function getCalleeInfo($trace, $num_params)
    {
        ...
        foreach ($trace as $index => $frame) {
            ...
            if (!Kint_Parser_Trace::frameIsListed($frame, ['spl_autoload_call'])) {
                $miniTrace[] = $frame;
            }
        }
        ...
        if (!isset($callee['file'], $callee['line']) || !is_readable($callee['file'])) {
            return [null, [], $callee, $caller, $miniTrace];
        }
        ...
        if (empty($callee['class'])) {
            ...
            $callfunc = [$callee['class'], $callee['function']];
        }
        ...
        foreach ($calls as $call) {
            ...
            if ($is_unpack || count($call['parameters']) === $num_params) {
                if ($return[0] === null) {
                    $return = [$call['parameters'], $call['modifiers'], $callee, $caller, $miniTrace];
                } else {
